...
package object stream {
    ...
    def extractFile(gitRepo: os.Path, filename: String): Either[String, Stream[String]] = {
        if (!os.exists(gitRepo)) Left(s"git repo dir does not exist: ${gitRepo.toString}")
            ...
            if (result.exitCode == 0)
                // lines() returns Vector[String] so we don't really get the lazy read benefit of Stream here
                Right(result.out.lines().toStream)
            else
                Left("error running git: "+result.err.text()+result.out.text())
        }
    }

    def readFile(file: os.Path): Either[String, Stream[String]] =
        if (os.exists(file)) Right(os.read.lines(file).toStream)
